...
- active UPnP event subscriptions
...
compatibility.
Root descriptors advertise event subscription URLs for ContentDirectory,
ConnectionManager, and MediaReceiverRegistrar.
...
errors. Browse and search support pagination plus multi-field UPnP sort
criteria.

UPnP eventing tracks subscription IDs, renewal timeouts, sequence counters,
expiry, and initial notifications. Library-visible updates increment
`SystemUpdateID` and publish event notifications; fragile renderer profiles can
disable eventing before the event handler accepts a subscription.

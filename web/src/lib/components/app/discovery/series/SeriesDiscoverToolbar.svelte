<script lang="ts">
	...
	const selectedSortKey = $derived(seriesSortKey(sort));
	const selectedDirection = $derived(seriesSortDirection(sort));
	const selectedSort = $derived(
		seriesSortOptions.find((option) => option.key === selectedSortKey) ?? seriesSortOptions[0]
	);

	function chooseSort(key: SeriesSortKey) {
		onSortChange(nextSeriesSort(sort, key));
	}
</script>

<div class="relative z-20 flex flex-wrap items-center justify-end gap-2">
	<Select.Root type="single" value={selectedSortKey}>
		<Select.Trigger class="min-w-[190px] justify-between" data-testid="series-discover-sort">
		...
		<Select.Content>
			{#each seriesSortOptions as option (option.key)}
				<Select.Item
					value={option.key}
					label={option.label}
	...
	<Button
		...
		data-testid="series-discover-filters"

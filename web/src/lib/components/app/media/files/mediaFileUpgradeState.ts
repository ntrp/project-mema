...
function qualityID(quality: string, formats: string[]) {
	...
	const source = normalizedFormats.includes('web-dl')
		? 'webdl'
		: normalizedFormats.includes('bluray')
			? 'bluray'
			: '';
	if (!source || !normalizedQuality.endsWith('p')) return undefined;
	return `${source}-${normalizedQuality}`;
}

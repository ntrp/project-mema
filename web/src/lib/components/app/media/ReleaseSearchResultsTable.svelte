<script lang="ts">
	...
	let { item, releases, releaseResults, sort, grabbingKey, canManage, onSort, onGrab }: Props =
		$props();
...
<Card class="max-h-[min(520px,calc(100vh-340px))] overflow-auto p-0">
	<Table.Root class="min-w-[1180px]">
		...
		<Table.Body>
				...
				<Table.Row>
					...
					<Table.Cell class="max-w-96">
						<Tooltip.Root>
							<Tooltip.Trigger>
								{#snippet child({ props })}
									<span {...props} class="block max-w-96 truncate">{release.title}</span>
								{/snippet}
							</Tooltip.Trigger>
					...
					<Table.Cell>
						<div class="flex max-w-56 flex-wrap gap-1">
							{#each languageLabels(release) as language (language)}
								<Badge variant="secondary" class="bg-muted text-muted-foreground">{language}</Badge>
							{/each}
						</div>
					</Table.Cell>
					<Table.Cell>
					...
					<Table.Cell class="text-right">
						{#if canManage}
							<Tooltip.Root>
								...
								<Tooltip.Content>
									{grabbingKey === releaseKey(release) ? 'Queueing release' : 'Grab release'}
								</Tooltip.Content>
							</Tooltip.Root>
						{/if}
					</Table.Cell>
			...
			{:else}
				<Table.Row>
					<Table.Cell colspan={11} class="text-muted-foreground">
						...
						{:else}
							{releaseResults?.loaded
								? 'No release candidates match the filters.'

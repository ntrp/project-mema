...
>
	<Card.Root class="min-h-0" aria-labelledby="indexer-cache-title">
		<Card.Header>
			<div>
			...
			<Card.Action>
				<Tooltip.Root>
					<Tooltip.Trigger>
						{#snippet child({ props })}
							<Button
								...
								variant="destructive"
								size="icon-sm"
								aria-label="Reset indexer query cache"
								disabled={clearing}
					...
					<Tooltip.Content>Reset indexer query cache</Tooltip.Content>
				</Tooltip.Root>
			</Card.Action>
		</Card.Header>
	...
	<Card.Root class="min-h-0" aria-labelledby="indexer-history-title">
		<Card.Header>
			<div>
				<Card.Description class="flex items-center gap-2">
					<span class="relative flex size-2.5">
			...
			<Card.Action>
				<Tooltip.Root>
					<Tooltip.Trigger>
						{#snippet child({ props })}
							<Button
								...
								variant="destructive"
								size="icon-sm"
								aria-label="Clear indexer query history"
					...
					<Tooltip.Content>Clear indexer query history</Tooltip.Content>
				</Tooltip.Root>
			</Card.Action>
		</Card.Header>
		<Card.Content class="min-h-0 flex-1">
			<IndexerSearchHistoryTable
				{search}
				{loading}

...
<Dialog.Root open={!!abortCandidate} onOpenChange={(open) => !open && (abortCandidate = undefined)}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>Abort job</Dialog.Title>
			<Dialog.Description>
				Abort job {abortCandidate?.id} ({abortCandidate?.kind})?
			</Dialog.Description>
		</Dialog.Header>
		<Dialog.Footer>
			<Button type="button" variant="outline" onclick={() => (abortCandidate = undefined)}>
				Cancel
			</Button>
			<Button type="button" variant="destructive" onclick={() => void abortJob()}>Abort</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>

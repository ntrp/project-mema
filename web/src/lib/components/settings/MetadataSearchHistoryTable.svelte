<script lang="ts">
	...
	const stats = $derived([
		{ label: 'Displayed entries', value: cache.historyEntries.length },
		{ label: 'Cache hits', value: cache.historyEntries.filter((entry) => entry.cacheHit).length },
		{
			label: 'Cache misses',
			value: cache.historyEntries.filter((entry) => !entry.cacheHit).length
		},
		{ label: 'Failures', value: cache.historyEntries.filter((entry) => !entry.success).length }
	]);
</script>
	...
	<div class="max-h-96 overflow-auto rounded-md border border-border">
		{#if cache.historyEntries.length > 0}
			<Table.Root class="min-w-[1040px]">
				...
				<Table.Body>
					{#each cache.historyEntries as entry (`${entry.createdAt}:${entry.providerName}:${entry.query}`)}
						<Table.Row>

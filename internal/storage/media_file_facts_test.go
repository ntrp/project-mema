...
func TestLiveFilesystemChangesNeedRescanToUpdateFileFact(t *testing.T) {
	ctx, store := testDBStore(t)
	item := rescanMediaItem(t, ctx, store)
	path := filepath.Join(*item.MediaFolderPath, "Scenario.Movie.2026.mkv")
	writeSizedFile(t, path, 23)
	...
	writeSizedFile(t, path, 41)
	facts, err := store.ListMediaFileFacts(ctx, item.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(facts) != 1 || facts[0].SizeBytes == nil || *facts[0].SizeBytes != 23 {
		t.Fatalf("facts changed before rescan = %#v", facts)
	}
	...
	if len(facts) != 1 || facts[0].SizeBytes == nil || *facts[0].SizeBytes != 41 {
		t.Fatalf("facts after rescan = %#v", facts)
	}
}

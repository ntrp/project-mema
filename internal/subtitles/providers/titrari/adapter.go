...
const key = "titrari"
const defaultBaseURL = "https://titrari.ro"
...
func (adapter) Search(ctx context.Context, svc providercore.Service, cfg providercore.Config, sr providercore.SearchRequest) ([]providercore.Candidate, error) {
	...
	searchURL := titrariSearchURL(cfg, sr)
	...
	return parseTitrari(doc.Selection, searchURL, sr.LanguageID)
}
...
func titrariSearchURL(cfg providercore.Config, sr providercore.SearchRequest) string {
	...
	return sitehtml.BaseURL(cfg, defaultBaseURL) + "/cauta?" + values.Encode()
}

func parseTitrari(root *goquery.Selection, pageURL, languageID string) ([]providercore.Candidate, error) {
	...
	root.Find("[data-subtitle], [data-subtitle], .subtitle, .result, table tr, li").Each(func(_ int, row *goquery.Selection) {
		href := downloadHref(row, "download", "descarca", ".zip", ".srt")
		if href == "" {
			return
		}
...
func downloadHref(row *goquery.Selection, needles ...string) string {
	href := ""

...
const key = "supersubtitles"
const defaultBaseURL = "https://feliratok.eu"
...
func (adapter) Search(ctx context.Context, svc providercore.Service, cfg providercore.Config, sr providercore.SearchRequest) ([]providercore.Candidate, error) {
	...
	searchURL := supersubtitlesSearchURL(cfg, sr)
	...
	return parseSuperSubtitles(doc.Selection, searchURL, sr.LanguageID)
}
...
func parseSuperSubtitles(root *goquery.Selection, pageURL, languageID string) ([]providercore.Candidate, error) {
	var candidates []providercore.Candidate
	root.Find("[data-subtitle], table tr, .lista tr, .subtitle, .result").Each(func(_ int, row *goquery.Selection) {
		href := ""
		row.Find("a[href]").EachWithBreak(func(_ int, link *goquery.Selection) bool {
			candidate, _ := link.Attr("href")
			lower := strings.ToLower(candidate)
			if strings.Contains(lower, "download") || strings.Contains(lower, "letoltes") || strings.HasSuffix(lower, ".srt") {
				href = candidate
				return false
			}
			return true
...
func supersubtitlesSearchURL(cfg providercore.Config, sr providercore.SearchRequest) string {
	base := sitehtml.BaseURL(cfg, defaultBaseURL)
	values := url.Values{"search": {sr.Title}}
	if sr.Year != nil {
		values.Set("ev", fmt.Sprint(*sr.Year))
	}
	if sr.SeasonNumber != nil {
		values.Set("sid", fmt.Sprint(*sr.SeasonNumber))
	}
	if sr.EpisodeNumber != nil {
		values.Set("eid", fmt.Sprint(*sr.EpisodeNumber))
	}
	return base + "/index.php?" + values.Encode()
}

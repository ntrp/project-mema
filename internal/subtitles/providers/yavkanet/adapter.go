...
const key = "yavkanet"
const defaultBaseURL = "https://yavka.net"
...
func (adapter) Search(ctx context.Context, svc providercore.Service, cfg providercore.Config, sr providercore.SearchRequest) ([]providercore.Candidate, error) {
	...
	searchURL := sitehtml.BaseURL(cfg, defaultBaseURL) + "/search?" + values.Encode()
	...
	return parseYavka(doc.Selection, searchURL, sr.LanguageID)
}
...
func parseYavka(root *goquery.Selection, pageURL, languageID string) ([]providercore.Candidate, error) {
	var candidates []providercore.Candidate
	root.Find("[data-subtitle], [data-subtitle], .subtitle, .result, article, table tr, li").Each(func(_ int, row *goquery.Selection) {
		href := ""
		row.Find("a[href]").EachWithBreak(func(_ int, link *goquery.Selection) bool {
			candidate, _ := link.Attr("href")
			lower := strings.ToLower(candidate)
			if strings.Contains(lower, "download") || strings.Contains(lower, "subtitle") || strings.HasSuffix(lower, ".srt") {
				href = candidate
				return false
			}
			return true

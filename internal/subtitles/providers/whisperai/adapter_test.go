...
func TestSearchDetectsLanguageAndBuildsTranscribeCandidate(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		detectPath = r.URL.Path; contentType = r.Header.Get("Content-Type")
	...
	if detectPath != "/detect-language" || !strings.HasPrefix(contentType, "multipart/form-data") { t.Fatalf("detect request = %s %s", detectPath, contentType) }
	if len(got) != 1 || got[0].LanguageID != "spa" || !strings.Contains(got[0].ReleaseName, "transcribe spa") { t.Fatalf("candidates = %#v", got) }
}
...
func TestSearchOnlyTranslatesToEnglish(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(got) != 0 { t.Fatalf("non-English translation candidates = %#v", got) }
	...
	if err != nil { t.Fatal(err) }
	if len(got) != 1 || !strings.Contains(got[0].ReleaseName, "translate spa audio -> eng") { t.Fatalf("English candidate = %#v", got) }
}

func TestDownloadPostsMultipartASRWithSettings(t *testing.T) {
	var path, query, contentType string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		path = r.URL.Path; query = r.URL.RawQuery; contentType = r.Header.Get("Content-Type")
		_, _ = io.ReadAll(r.Body)
		_, _ = w.Write([]byte("1\n00:00:01,000 --> 00:00:02,000\nHola"))
	}))
	defer server.Close()
	config := providercore.Config{BaseURL: server.URL, Settings: map[string]providercore.SettingValue{"passVideoName": boolSetting(true)}, CommandRunner: whisperRunner(t)}
	...
	if err != nil { t.Fatal(err) }
	if path != "/asr" || !strings.Contains(query, "task=transcribe") || !strings.Contains(query, "language=es") || !strings.Contains(query, "video_file=%2Fvideo.mkv") { t.Fatalf("asr request = %s?%s", path, query) }
	if !strings.HasPrefix(contentType, "multipart/form-data") || !strings.Contains(string(got.Content), "Hola") { t.Fatalf("content type/content = %q %q", contentType, got.Content) }
}
...
func whisperRunner(t *testing.T) providercore.CommandRunner {
	...
	return func(_ context.Context, name string, args ...string) ([]byte, error) {
		...
		case "ffmpeg":
			if !strings.Contains(joined, "-f s16le") { t.Fatalf("ffmpeg args = %v", args) }
...
func strSetting(s string) providercore.SettingValue { return providercore.SettingValue{StringValue: &s} }
func boolSetting(b bool) providercore.SettingValue { return providercore.SettingValue{BooleanValue: &b} }
